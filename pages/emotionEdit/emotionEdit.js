...
Page({
  ...
  onLoad: function (option) {
    ...
    console.log(option.imageheight)
    console.log(z.data.imagewidth)
    wx.getSystemInfo({
      success: (result)=>{
        z.setData({
          canvaswidth:result.windowWidth,
          canvasheight:result.windowHeight*0.8
        })
      },
      fail: ()=>{},
      complete: ()=>{}
  ...
  competeAddText() {
    ...
    if (allText[allText.length - 1].someText == "点击输入文字" || allText[allText.length - 1].someText == "") {
      ...
      var width=parseInt(self.data.imagewidth)
      var height=parseInt(self.data.imageheight)//此时数据类型是string，需要转为number类型数据才能用
      ctx.drawImage(self.data.emotionUrl, 0, 0, width, height)
  ...
  saveImgToPhone(){
    const z=this
    // wx.previewImage({
    //   urls: [this.data.emotionUrl] // 需要预览的图片http链接列表
    // })
    wx.navigateTo({
      url:"/pages/list/list?chooseemotion="+z.data.tempImageSrc,
    })
  }
})
  ...
  setTimeout(function () {
    wx.canvasToTempFilePath({
      ...
      success: function (res) {
        ...
        // wx.navigateTo({
        //     url:"/pages/list/list?chooseemotion="+res.tempFilePath,
        //   })
      }
    })
  }, delay)
